...
class dataLoader_train(Dataset):
    def __init__(self, tr_num, root_txt, root_radar, root_lidar, batch_size, img_H, img_W, shuffle=True):
        ...
        self.tr_num = tr_num
        # from zero
        index_all = np.arange(self.tr_num)
        index_all = [x+1 for x in index_all]

        # shuffle
        if shuffle:
            np.random.shuffle(index_all)
        self.index_tr = index_all[0:self.tr_num]
    ...
    def get_data(self, iter, device):
        ...
        data_negative_1_radar = torch.zeros(self.batch_size, 1, self.img_H, self.img_W).to(device)
        data_negative_1_lidar = torch.zeros(self.batch_size, 1, self.img_H, self.img_W).to(device)

        data_negative_2_radar = torch.zeros(self.batch_size, 1, self.img_H, self.img_W).to(device)
        data_negative_2_lidar = torch.zeros(self.batch_size, 1, self.img_H, self.img_W).to(device)
        ...
        for i in range(self.batch_size):
            ...
            data_negative_1_radar[i] = self.read_txt(self.root_radar+str(line_list[2])+'.txt')
            data_negative_1_lidar[i] = self.read_txt(self.root_lidar+str(line_list[2])+'.txt')

            data_negative_2_radar[i] = self.read_txt(self.root_radar+str(line_list[3])+'.txt')
            data_negative_2_lidar[i] = self.read_txt(self.root_lidar+str(line_list[3])+'.txt')

        data_radar_batch = torch.cat((data_anchor_radar,data_positive_radar,data_negative_1_radar, data_negative_2_radar),0)
        data_lidar_batch = torch.cat((data_anchor_lidar,data_positive_lidar,data_negative_1_lidar, data_negative_2_lidar),0)

function [] = save_training_txt_random_quad(pose, save_txt, sample_num, start_id, end_id)
% YH
...
for i = 1:sample_num
    ...
    while(flag == 1)
        if close_flag == 1
            negative_id_1(i) = randi([ancher_id(i) - shocking_range,...
                ancher_id(i) + shocking_range], 1, 1);
            negative_id_2(i) = randi([ancher_id(i) - shocking_range,...
                ancher_id(i) + shocking_range], 1, 1);
        else
            negative_id_1(i) = randi([start_id, end_id], 1, 1);
            negative_id_2(i) = randi([start_id, end_id], 1, 1);
        end
        
        pose_neg_1 = pose_xy(negative_id_1(i), :);
        pose_neg_2 = pose_xy(negative_id_2(i), :);
        
        if norm(pose_neg_1 - pose_cur) > neg_th && ...
                norm(pose_neg_2 - pose_cur) > neg_th && ...
                norm(pose_neg_1 - pose_neg_2) > neg_th 
            flag = 0;
        else
            flag = 1;
...
labels = [ancher_id', postive_id', negative_id_1', negative_id_2'];

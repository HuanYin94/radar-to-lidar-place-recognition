...
class quad_loss(torch.nn.Module):
    ...
    def forward(self, radar_feature):
        ...
        fft_negative_1_radar = radar_fft[2*self.batch_size:3*self.batch_size-1,:,:,:].reshape(self.batch_size,-1)

        fft_negative_2_radar = radar_fft[3*self.batch_size:4*self.batch_size-1,:,:,:].reshape(self.batch_size,-1)

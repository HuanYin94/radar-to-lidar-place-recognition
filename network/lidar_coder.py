...
class lidar_coder(nn.Module):
    def __init__(self):
        super(lidar_coder, self).__init__()

        self.lidar_nn = unet(1,64)
        self.feat_net_lidar = nn.Conv2d(64, 1, kernel_size=1)

    def forward(self, lidar_sc_batch):

        # feature for both scan & map
        lidar_batch_feature = self.feat_net_lidar(self.lidar_nn(lidar_sc_batch))

        return lidar_batch_feature